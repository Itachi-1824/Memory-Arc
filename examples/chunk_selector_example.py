"""Example demonstrating priority-based chunk selection."""
...
from core.infinite.chunk_selector import ChunkSelector, ScoredChunk
...
def main():
    ...
    # Example 6: Selection with associated memories
    print("\n\n6. SELECTION WITH ASSOCIATED MEMORIES")
    print("-" * 70)
    
    # Create memories aligned with chunks
    memories = [
        Memory(
            id=f"mem_{i}",
            context_id="user_123",
    ...
    print(f"Query: '{query}'")
    print(f"\nTop 3 chunks (with memory metadata):")
    for i, sc in enumerate(scored_chunks, 1):
        print(f"\n{i}. {sc.chunk.id} - Final Score: {sc.final_score:.3f}")
        print(f"   Content: {sc.chunk.content[:60]}...")
        print(f"   Memory importance: {memories[int(sc.chunk.id.split('_')[1]) - 1].importance}")
    
    # Example 7: Custom recency decay
    print("\n\n7. CUSTOM RECENCY DECAY")
    print("-" * 70)
